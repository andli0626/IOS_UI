...
@implementation UserWebServiceHelper
@synthesize funName;
@synthesize funpars;
@synthesize delegate;
...
+(NSString *)getWSDLURL
{
    return [NSString stringWithFormat:@"%@%@",my_user_web_address,my_user_web_service];
}

+(NSString*)getUploadURL
{
    return [NSString stringWithFormat:@"%@%@/%@",my_user_web_address,my_user_web_service,@"upload"];
}
//返回Soap Body
+(NSString *)getSoapBody:(NSString *)funcationName parameter:(NSString *)content
{
    ...
    return [NSString stringWithFormat:@"<?xml version=\"1.0\" encoding=\"utf-8\"?><soap:Envelope xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" xmlns:xsd=\"http://www.w3.org/2001/XMLSchema\" xmlns:soap=\"http://schemas.xmlsoap.org/soap/envelope/\"><soap:Body>"
            ...
            "%@"
            "</%@>"
            "</soap:Body></soap:Envelope>",funcationName,my_user_web_namespace,content,funcationName];
}
...
+(NSMutableURLRequest *)getRequest:(NSString *)funcationName parameter:(NSString *)pars
{
    ...
    NSString* funurl = [NSString stringWithFormat:@"%@%@",my_user_web_namespace,funcationName];
...
{
    NSMutableURLRequest* request= [UserUploadNSRequest uploading:image message:@"hello ljh"];
    isRequesting = YES;
...
-(void)responseFail
{
    ...
    {
        [delegate requestUserFailed:self];
    }
}
-(void)responseFinished
{
    if([delegate respondsToSelector:@selector(requestUserFinished:)])
    {
        [delegate requestUserFinished:self];
    }
}
-(void)startSynchronous
{
    ...
    NSMutableURLRequest* request = [UserWebServiceHelper getRequest:funName parameter:funpars];
    isRequesting = YES;
...
@implementation UserUploadNSRequest
...
+(NSMutableURLRequest *)uploading:(UIImage *)image message:(NSString *)mes
{
    ...
    NSString *urlString = [UserWebServiceHelper getUploadURL];
